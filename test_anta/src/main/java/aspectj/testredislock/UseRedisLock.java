...
public class UseRedisLock {
    ...
    @RedisLocked(key = "grantInstance_args[0].userId_args[0].projectId", returnValue = @RedisLocked.ReturnValue(value = "{\"message\":\"该用户正在发放或领取优惠券中，请稍后重试\"}", valueClass = DataResult.class), blockTime = @RedisLocked.BlockTime(10))
